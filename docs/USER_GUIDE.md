...
Processing path ไม่มี network API และโมเดลถูกฝังในแอปทั้งหมด Release app ใช้พื้นที่ประมาณ 2.1 GB Generative Fill ใช้หน่วยความจำมากที่สุดและโหลดโมเดลแบบ low-memory ทีละงาน การเช็กอัปเดตเป็น network path เดียว ปิดได้ใน Settings และดาวน์โหลดเฉพาะ stable GitHub Release ที่มี SHA-256 คู่กัน
